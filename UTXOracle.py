...
#                                                                                       #
# Inspired by the original UTXOracle concept by @SteveSimple                           #
#########################################################################################                                     
...
class BitcoinRPC:
    def __init__(self, bitcoin_cli_path: str = "./bitcoin-cli", cli_options=None):
        self.bitcoin_cli = bitcoin_cli_path
        self.cli_options = cli_options or []

    def call(self, command: list, expect_json: bool = True):
        try:
            result = subprocess.run(
                [self.bitcoin_cli] + self.cli_options + command,
                capture_output=True,
                text=True,
                timeout=30
            )
            if result.returncode == 0:
                ...
                print(f"Erreur RPC: {result.stderr}")
                return None
        except subprocess.TimeoutExpired:
            print("Timeout lors de l'appel RPC")
            return None
    ...
    def get_block_hash(self, height: int) -> str:
        return self.call(["getblockhash", str(height)], expect_json=False)

    def get_block(self, block_hash: str, verbosity: int = 1):
        # verbosity=1: block info + txids, 2: block info + full txs, 0: raw hex
        return self.call(["getblock", block_hash, str(verbosity)], expect_json=True)

    def get_block_count(self) -> int:
        return int(self.call(["getblockcount"], expect_json=False))
...
# Default Bitcoin data directory paths
UMBREL_BITCOIN_DATA_DIR = "/home/umbrel/umbrel/app-data/bitcoin-knots/data/bitcoin"
...
if system == "Darwin":  # macOS
    ...
    os.system('cls')
else:  # Linux or others (default Umbrel path, can be overridden with -p)
    data_dir = os.path.expanduser(UMBREL_BITCOIN_DATA_DIR)
    os.system('clear')
...
found_blocks = {}
print_next = 0
for i, block_hash in enumerate(block_hashes_needed):
    block_info = rpc.get_block(block_hash, verbosity=1)
    if block_info:
...
print("\nLoading every transaction from every block (via RPC)", flush=True)
...
print_next = 0
block_num = 0
input_txids_by_block = defaultdict(set)

for i, (block_hash, meta) in enumerate(found_blocks.items()):
    block_num += 1
    txs_to_add = []
    ...
    for tx in block_data["tx"]:
        ...
        # Witness data (taille totale)
        witness_exceeds = False
        total_witness_len = 0
        for vin in tx.get("vin", []):
            witness = vin.get("txinwitness", [])
            for w in witness:
                total_witness_len += len(bytes.fromhex(w)) if isinstance(w, str) else 0
                if isinstance(w, str) and len(w) > 500:
        ...
        if (
            ...
            for vout in tx.get("vout", []):
                value_btc = vout.get("value", 0.0)
                if 1e-5 < value_btc < 1e5:
                    raw_outputs.append(value_btc)
                    block_heights_dec.append(meta["height"])
                    block_times_dec.append(meta["time"])
                    txs_to_add.append(value_btc)
        if txid:
...
# Placer chaque output dans le bon bin de la courbe
for amount in raw_outputs:
...
print("curve_sum:", curve_sum)
print("Non-zero bins:", sum(1 for x in output_bell_curve_bin_counts if x > 0))
